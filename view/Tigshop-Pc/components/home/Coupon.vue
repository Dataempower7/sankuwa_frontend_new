            ...
            <div class="item" v-for="(item, index) in couponList" :key="index">
                <a>
                    <div class="coupon_detail">
                        ...
                        <div class="direction">
                            <el-tooltip effect="light" :content="$t(item.couponName)" :disabled="item.couponName && item.couponName.length < 5" placement="top">
                                <p class="coupon_sort">{{ $t(item.couponName) }}</p>
                            </el-tooltip>

                            <p>{{ $t(item.couponDesc) }}</p>
                        </div>
                    </div>
...
const getData = async () => {
    try {
        ...
        couponList.value = result.slice(0, 5);
    } catch (error) {
    } finally {
    }
};

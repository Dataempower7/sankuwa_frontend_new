            ...
            <div style="background-color: #f2f3f5">
                ...
                <div v-if="homeStore.moduleList.length > 0">
                    <template v-for="(item, index) in homeStore.moduleList" :key="index">
                        <HomeBanner v-if="item.type == 'pcBanner'" v-model="item.module" :class="{ container: item.module.bannerStyle == 1 }" />
                    </template>
                </div>
                <div class="container">
                    <div v-if="homeStore.moduleList.length > 0" class="home-warp">
                        <template v-for="(item, index) in homeStore.moduleList" :key="index">
                            <HomeCatWide :moduleIndex="item.moduleIndex"  v-if="item.type === 'pcCatWide'"/>
                            <HomeSeckill v-if="item.type == 'pcSeckill'" v-model="item.module" />
                            <HomeCoupon v-if="item.type == 'pcCoupon'" v-model="item.module" />
                            <HomeCatProductNormal v-if="item.type == 'pcCatProductNormal'" v-model="item.module" />
                            <HomeCatProductWide v-if="item.type == 'pcCatProductWide'" v-model="item.module" />
                            <HomeCatProductSimple v-if="item.type === 'pcCatProductSimple'" v-model="item.module" :moduleIndex="item.moduleIndex" />
                            <HomeCustomAd1 v-if="item.type == 'pcCustomAd1'" v-model="item.module" />
                            <HomeCustomAd2 v-if="item.type == 'pcCustomAd2'" v-model="item.module" />
                            <HomeImageAd v-if="item.type == 'pcImageAd'" v-model="item.module" />
                            <HomeRecommendProduct
                                v-if="item.type == 'pcRecommendProduct'"
                                v-model="item.module"

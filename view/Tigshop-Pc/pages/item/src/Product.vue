<template>
    <div style="background: #fff" v-if="!loading">
        <CommonPageHeader ref="pageHeaderRef" :immediateLog="false" :showService="true"
            :shopId="shopInfo && shopInfo.shopId ? shopInfo.shopId : 0"
            :phone="shopInfo && shopInfo.kefuPhone ? shopInfo.kefuPhone : ''" :productId="productId ?? 0">
        </CommonPageHeader>
        <el-dialog v-model="visible" :title="$t('分享给小伙伴')" :footer="null" top="15vh" width="800px">
            <div>
                <ProductShareModal :picList="picList" :product="product" :productPrice="productPrice">
                </ProductShareModal>
            </div>
        </el-dialog>
        <div class="container">
            <div>
                <CommonCategoryNavigation v-model:id="product.categoryId" :productName="product.productName">
                </CommonCategoryNavigation>
                <div class="product_details flex justify-between" v-if="!loading">
                    <div class="extra_details">
                        ...
                        <div class="share_wish flex" v-if="productType == 'product' || productType == 'exchange'">
                            ...
                            <span class="iconfont-pc collect ico-color"
                    ...
                    <div class="details">
                        ...
                        <ProductInfo :product="product" :serviceList="serviceList" :productType="productType"
                            :id="productId" :productPrice="productPrice" :attrList="attrList" :skuList="skuList"
                            :b2bSkuList="b2bSkuList" :productStock="productStock" :productNumber="productNumber"
                            :rankDetail="rankDetail" :descArr="descArr" :checkedValue="checkedValue"
                            :exchangeDetail="exchangeDetail"></ProductInfo>
                    </div>
                </div>
                <div class="detail_main flex">
                    <template v-if="productType == 'product' || productType == 'exchange'">
                        <div class="lyecs_side">
                            <template v-if="shopInfo && isMerchant()">
                                <div class="shop-box">
                                    <div class="tit">
                                        <NuxtLink :to="urlFormat({ path: 'shop', id: shopInfo.shopId })">{{
                                            shopInfo.shopTitle }}</NuxtLink>
                                    </div>
                    ...
                    <div class="lyecs_main">
                            ...
                            <div class="tabs">
                                <ul class="flex">
                                    <li @click="tabChange(1)">
                                        <a href="javascript:;" tab-data="info" class="tab_a"
                                            :class="tabIndex == 1 ? 'current' : ''"><span>{{ $t("产品详情") }}</span></a>
                                    </li>

                                    <li @click="tabChange(2)">
                                        <a href="javascript:;" tab-data="com" class="tab_a"
                                            :class="tabIndex == 2 ? 'current' : ''"><span>{{
                                                $t("用户评价") }}<em>({{ rankDetail.total }})</em></span></a>
                                    </li>
                                    <li @click="tabChange(3)">
                                        <a href="javascript:;" tab-data="ask" class="tab_a"
                                            :class="tabIndex == 3 ? 'current' : ''"><span>{{
                                                $t("商品咨询") }}<em>({{ consultationTotal }})</em></span></a>
                                    </li>
                                    <li @click="tabChange(4)">
                                        <a href="javascript:;" tab-data="ser" class="tab_a"
                                            :class="tabIndex == 4 ? 'current' : ''"><span>{{
                                                $t("售后服务") }}</span></a>
                                    </li>
                                </ul>
                            </div>
                                ...
                                <div class="goods_description">
                                    <template v-if="product.isBuy && product.paidContent">
                                        <template v-for="item in product.paidContent" :key="item">
                                            <img class="el-img" v-if="item.type == 'pic'" fit="cover"
                                                :src="imageFormat(item.pic)" />
                                            <div class="desc-text-item" v-if="item.type == 'text'"
                                                v-html="formatRichText(item.html)"></div>
                                        </template>
                                    </template>
                                    <template v-else>
                            ...
                            <template v-if="attrList.normal && attrList.normal.length > 0">
                                <ul class="shuxin flex">
                                    <template v-for="(item, index) in attrList.normal" :key="index">
                                        <li>
                                            {{ item.attrName }}：
                                            <template v-for="(attr, arrtIndex) in item.attrList" :key="arrtIndex"> {{
                                                attr.attrValue }} &nbsp; </template>
                                        </li>
                                    </template>
                                </ul>
                            </template>
                        ...
                        <div class="goods_desc goods_ask" v-if="tabIndex == 3" id="zixun">
                            ...
                            <ProductConsultation :product="product" :productStock="productStock"
...
const getProduct = async () => {
    try {
        ...
        if (product.value.shopId) {
            await getShop(product.value.shopId);
        }

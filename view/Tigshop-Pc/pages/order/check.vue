            ...
            <div class="orderConfirmCon">
                                ...
                                <div class="from">
                                    <div class="check-box flex align-center">
                                        <el-checkbox :checked="formState.usePoint > 0" @click="onUsePoint">
                                            <span class="tit">{{ $t("使用积分") }}</span></el-checkbox
                                        >
                                        <div style="margin-left: 10px">
                                            {{ $t("当前账户共") }} <span class="font-color"> {{ userStore.userInfo.points }} </span> {{ $t("积分") }}
                                        </div>
                                    </div>
                                </div>
                            </template>
                            ...
                            <div class="from_list" v-if="showUsePoint">
                                ...
                                <div class="integral flex">
                                    ...
                                    <div class="inp_box">
                                        ...
                                        <div class="tips">
                                            {{ $t("该订单最多可用") }}{{ availablePoints }} {{ $t("积分") }}
                                            <a href="/article/issue/info?articleId=7" target="_blank"
                                                ><span class="font-color">【{{ $t("如何获得积分") }}？】</span></a
                                            >
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="money_info">
                    <ClientOnly>
                        <div class="total_list">
                            ...
                            <div class="item" v-if="total.pointsAmount > 0 && flowType != 3" style="display: flex; align-items: center">
                                <span style="display: inline-flex; align-items: center">-</span>
                                <span style="margin-left: 4px">{{ $t("积分减免") }}：</span>
                    ...
                    <div v-if="total.orderSendPoint && total.orderSendPoint > 0" class="complete">
                        *{{
                            isOverseas()
                                ? $t("该订单完成后，您将预计获得{0}积分", [total.orderSendPoint])
                                : `该订单完成后，您将预计获得${total.orderSendPoint}积分`
                        }}
                    </div>
                        ...
                        <template v-if="flowType == 3">
                            <div class="points-box">
                                <div class="points-value">
                                    ...
                                    <div class="points-text">{{ $t("积分") }}</div>
                                </div>
...
const titleText = computed(() => {
    const arr = [commonStore.useCoupon == 1 && "优惠券", commonStore.usePoints == 1 && "积分", commonStore.useSurplus == 1 && "余额"].filter(Boolean);
...
const onUsePoint = () => {
    ...
    if (formState.usePoint == 0) return;
    updateOrderCheckData();
};

            ...
            <div class="lyecs-table-list-warp">
                            ...
                            <div class="simple-form-field">
                                <div class="form-group">
                                    <div class="control-container">
                                        <TigInput v-model="filterParams.keyword" name="keyword" :placeholder="'输入账号名称'"
                ...
                <div class="table-container">
                    <a-spin :spinning="loading">
                        <el-table :data="filterState" :loading="loading" :total="total" row-key="adminId"
                            ...
                            <el-table-column label="门店数" prop="storeCount" align="center">
                                <template #default="{ row }">
                                    <router-link v-if="row.storeCount"
                                        :to="{ path: '/organize/store/list', query: { account: row.username } }">
                                        {{ row.storeCount }}
                                    </router-link>
                                    <span v-else>-</span>
                                </template>
                            ...
                            <el-table-column label="供应商数" prop="vendorCount" align="center">
                                <template #default="{ row }">
                                    <router-link v-if="row.vendorCount"
                                        :to="{ path: '/organize/suppliers/list', query: { account: row.username } }">
                                        {{ row.vendorCount }}
                                    </router-link>
                                    <span v-else>-</span>
                                </template>
                            ...
                            <el-table-column :width="180" fixed="right" label="操作">
                                <template #default="{ row }">
                                    <DialogForm :params="{ act: 'detail', adminId: row.adminId }" isDrawer
                                        path="authority/accountManage/AccountInfo" title="修改账号信息" width="600px"
                                        @okCallback="loadFilter">
                                        <a class="btn-link">修改账号</a>
                                    </DialogForm>
                                    <el-divider direction="vertical" />
                                    <DialogForm :params="{ act: 'detail', adminId: row.adminId }" isDrawer
                                        path="authority/accountManage/Password" title="修改密码" width="600px"
                                        @okCallback="loadFilter">

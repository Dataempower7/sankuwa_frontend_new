...
const payTypeList = reactive([
    ...
    { value: "yabanpay_wechat", label: "YaBandPay微信" },
    { value: "yabanpay_alipay", label: "YaBandPay支付宝" },
    { value: "yunpay_wechat", label: "云支付微信" },
    { value: "yunpay_alipay", label: "云支付支付宝" },
    { value: "yunpay_yunshanfu", label: "云支付云闪付" }
]);

const payTypeName = (payType: string) => {
    const payTypeObj = payTypeList.find((item) => item.value === payType);
    return payTypeObj ? payTypeObj.label : "";
};
...
const handlePrint = async (ids: number[]) => {
    loadingPrint.value = true;

    try {
        let res = await triggerPrint({ ids: ids });

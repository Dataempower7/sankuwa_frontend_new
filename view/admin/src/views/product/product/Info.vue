<template>
    <div class="product-container">
        <div class="anchor">
            <el-anchor :container="containerRef" :offset="10" direction="horizontal" @change="onChange"
                @click="handleClick">
                <div class="tab-row">
                    <el-anchor-link v-for="(item, index) in tabList" :key="item.id"
                        :class="{ active: partTab === '' && index === 0 ? true : partTab === '#' + item.id }"
                        :href="'#' + item.id">
                        <template #default>
                            <div :class="{ active: partTab === '' && index === 0 ? true : partTab === '#' + item.id }"
                                class="item">
                                {{ item.title }}
                            </div>
                        </template>
        ...
        <div ref="containerRef" class="form-container">
            <el-form v-if="!loading" ref="formRef" :model="formState" label-width="110px" name="form_in_modal">
                <div id="part1" class="form-warp">
                    ...
                    <BasicInfo :type="type" :id="id" :shopId="shopId" :examine="examine" :loading="loading"
                        :action="action" :storeSettingInfo="storeSettingInfo" v-model:form-state="formState"
                        :formRef="formRef">
                    </BasicInfo>
                </div>
                <div id="part2" class="form-warp">
                    ...
                    <PriceInfo v-if="type !== 'vendor'" ref="priceInfoRef" :id="id" :shopId="shopId" :act="act"
                        :examine="examine" :attrTplList="attrTplList" v-model:isLimit="isLimit" :loading="loading"
                        v-model:form-state="formState" :storeSettingInfo="storeSettingInfo"></PriceInfo>
                    <VendorPriceInfo v-if="type == 'vendor'" ref="priceInfoRef" :id="id" :shopId="shopId" :act="act"
                        :examine="examine" :attrTplList="attrTplList" v-model:isLimit="isLimit" :loading="loading"
                        v-model:form-state="formState"></VendorPriceInfo>
                </div>
                <div id="part3" class="form-warp">
                    ...
                    <LogisticsInfo v-if="
                        type !== 'vendor' || (type == 'vendor' && formState.vendorProductId > 0 && formState.productType != 5 && formState.productType != 6)
                    " :id="id" :shopId="shopId" :examine="examine" :loading="loading" v-model:form-state="formState"
                        :act="act">
                    </LogisticsInfo>
                    <!-- 预约类型对应组件 -->
                    <OrderInfo v-model:form-state="formState"
                        v-if="formState.productType == 5 || formState.productType == 6" />
                </div>
                <div id="part4" class="form-warp">
                    ...
                    <DescInfo :type="type" :id="id" :shopId="shopId" :examine="examine" :loading="loading"
                        v-model:form-state="formState"></DescInfo>
                </div>
                <div id="part5" class="form-warp">
                    ...
                    <AdvancedInfo :id="id" :shopId="shopId" :examine="examine" :loading="loading"
                        v-model:form-state="formState"></AdvancedInfo>
                </div>
            </el-form>
        </div>
    </div>
...
import { BasicInfo, PriceInfo, LogisticsInfo, DescInfo, AdvancedInfo, VendorPriceInfo, OrderInfo } from "@/views/product/product/src/components";
...
const onSubmit = async () => {
    try {
        ...
        if (formState.value.productType !== 3 && formState.value.productType !== 5 && formState.value.productType !== 6) {

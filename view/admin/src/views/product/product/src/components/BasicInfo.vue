<template>
    <el-form-item :rules="[{ required: true }]" label="商品类型" prop="productType">
        <div>
            ...
            <RadioType v-model:modelValue="formState.productType" :disabled="action == 'detail'" :radioList="radioList">
            </RadioType>
    ...
    <el-form-item :rules="[{ required: true, validator: validateImg }]" label="商品图片" prop="imgList">
        <FormAddGallery v-if="!loading" v-model:photos="formState.imgList" :disabled="examine == 1 || type !== ''"
            :isMultiple="true" pageType="product"></FormAddGallery>
        <div class="extra">您可以通过拖拽来调整相册图片顺序，第一张图将作为商品主图展示</div>
    </el-form-item>
    <el-form-item label="商品视频" prop="productVideoInfo">
        <FormAddGalleryVideo v-if="!loading" v-model:video="formState.productVideoInfo"
            :disabled="examine == 1 || type !== ''"></FormAddGalleryVideo>
        <div class="extra">如有商品视频则商品视频作为商品主图展示</div>
    </el-form-item>
    <el-form-item v-if="formState.productType == 3" :rules="[{ required: true, validator: validateCardGroup }]"
        class="inner-item" label="电子卡券分组" prop="cardGroupId">
        <div class="flex flex-justify-between">
            <el-select v-model="formState.cardGroupId" :disabled="examine == 1 || type !== ''" placeholder="请选择"
                style="width: 200px">
                <el-option v-for="item in formState.eCardList" :key="item.groupId" :label="item.groupName"
                    :value="item.groupId" />
            </el-select>
        </div>
    </el-form-item>
    <el-form-item :rules="[{ required: true, validator: validateName }]" class="inner-item" label="商品名称"
        prop="productName">
        <BusinessData v-model:modelValue="formState.productName" :dataId="id" :dataType="2" :disabled="(getShopType() !== 2 && examine == 1) ||
            type !== '' ||
            (getShopType() === 2 && storeSettingInfo.storeAssignProductName === 0 && formState.shopId === 0)
            " :maxlength="100" placeholder="请输入商品名称"></BusinessData>
    </el-form-item>
    ...
    <el-form-item class="inner-item" prop="seoIdentification" v-if="isOverseas()"
                ...
                <el-tooltip class="box-item" effect="light" placement="bottom" show-after="300">
                    <template #content>
                        <div style="width: 300px; padding: 5px 10px">
                            SEO链接是商品页面的URL，用于搜索引擎优化，格式为：域名/product/[seo链接].[sn]
                        </div>
                    </template>
        ...
        <div class="keywords">
            <TigInput classType="tig-form-input" class="InputBox" v-model="formState.seoIdentification" />
            <el-button @click="onGenerateSeoLink" style="margin-top: 2px" :loading="seoLoading"
                :disabled="!formState.productName">自动生成</el-button>
        </div>
        <div class="extra">{{
            `SEO链接预览：域名/product/${formState.seoIdentification ? formState.seoIdentification :
                formState.productName ? formState.productName : '[seo链接]'}.[sn]`
        }}</div>

    </el-form-item>

    <el-form-item class="inner-item" label="商品品牌" prop="brandId">
        <SelectBrand v-if="!loading" v-model:brandId="formState.brandId" :disabled="examine == 1 || type !== ''"
            style="width: 200px"></SelectBrand>
    </el-form-item>
    <el-form-item :rules="[{ required: true, message: '请选择商品类目!', validator: validateCate }]" class="inner-item"
        label="商品类目" prop="categoryId">
        <div class="flex flex-justify-between">
            <SelectCategory v-if="!loading" ref="selectCategory" v-model:categoryId="formState.categoryId"
                :disabled="examine == 1 || type !== ''" :multiple="false" style="width: 200px"></SelectCategory>
            <div v-if="examine != 1" class="con-btn">
                <template v-if="getAdminType() === 'admin'">
                    <DialogForm :params="{ act: 'add', parentId: 0 }" isDrawer path="product/category/Info" title="添加分类"
                        width="600px" @okCallback="refreshCategory">
    ...
    <el-form-item v-if="getAdminType() == 'shop' && getShopType() !== 2" class="inner-item" label="店铺商品分类"
        prop="shopCategoryId">
        <SelectShopProductCategory v-model:categoryId="formState.shopCategoryId" :disabled="examine == 1">
        </SelectShopProductCategory>
    </el-form-item>
    <el-form-item class="inner-item" label="商品重量" prop="decimal" v-if="type == ''">
        <TigInput v-model="formState.productWeight" :decimalPlaces="3" :disabled="examine == 1" class="InputBox"
            placeholder="请输入" type="decimal" width="200px">
    ...
    <el-form-item class="inner-item" label="搜索关键词" prop="keywords">
        <div class="keywords">
            <TigInput v-model="formState.keywords" :disabled="examine == 1 || type !== ''" class="InputBox"
                classType="tig-form-input" placeholder="例如：牙刷 自动 清洁" />
    ...
    <el-form-item class="inner-item" extra="" label="商品描述" prop="productBrief">
        <div>
            <TigInput v-model="formState.productBrief" :disabled="examine == 1 || type !== ''" :rows="2"
                classType="tig-form-input" placeholder="例如：顺丰快递 又快又好" type="textarea"></TigInput>
            <div class="extra">
    ...
    <el-form-item class="inner-item" label="是否上架" prop="productStatus">
        <el-radio-group v-model="formState.productStatus" :disabled="examine == 1 && getShopType() !== 2"
            style="width: 100%">
...
import { isPro, isStore, isOverseas } from "@/utils/version";
...
onMounted(async () => {
    ...
    if ((isStore() && getAdminType() === "admin") || (getAdminType() === "shop" && getShopType() > 1)) {
        radioList.value.push({ key: 5, title: "预约上门", desc: "无需物流" }, { key: 6, title: "预约到店", desc: "无需物流" });
    }
});
// 会员名称
...
const onGenerateSeoLink = async () => {
    try {
        ...
        const result = await getTranslateToSeoIdentification(formState.value.productName || "");
...
const validateSeoIdentification = (rule: any, value: any, callback: any) => {
    if (!formState.value.productName && !value) {
        callback(new Error("请先输入商品名称"));
        return;
    }
    callback();
};

watch(() => formState.value.productName, (newVal: string | undefined) => {
    if (newVal) {
        console.log(newVal, 'newVal');
        //校验seoIdentification
        props.formRef.validateField("seoIdentification");
    }

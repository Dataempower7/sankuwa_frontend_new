        ...
        <div class="content_wrapper">
            <div class="lyecs-form-table">
                <el-form v-if="!loading" ref="formRef" :model="formState" label-width="auto">
                    <el-form-item v-if="getAdminType() == 'admin' && action == 'add'" label="所属商户" prop="merchantId"
                        :rules="[{ required: true, message: '请选择所属商户!' }]">
                        <DialogForm v-if="!isSpecified"
                            :params="{ isMultiple: false, selectedIds: [formState.merchantId] }" isDrawer
                            path="adminMerchant/merchant/src/SelectMerchant" title="选择商户" width="1100px"
                            @okCallback="callback">
                        ...
                        <el-table v-if="formTable.length > 0" :data="formTable" row-key="merchantId" class="mt10">
                            ...
                            <el-table-column label="商户所属会员(ID)">
                                <template #default="{ row }"> {{ row.user?.username }}{{ row.user?.userId ? "(" +
                                    row.user?.userId + ")" : "" }} </template>
                            </el-table-column>
                    ...
                    <el-form-item label="门头封面" prop="shopCoverPicture"
                        :rules="[{ required: true, message: '请上传门头封面!' }]">
                    ...
                    <el-form-item label="门店相册" prop="shopShowPicture">
                        <FormAddGallery isMultiple v-if="!loading" v-model:photos="formState.shopShowPicture">
                        </FormAddGallery>
                        <div class="extra">
                    ...
                    <el-form-item label="门店名称" prop="shopTitle" :rules="[{ required: true, message: '门店名称不能为空!' }]">
                        <TigInput classType="tig-form-input" v-model="formState.shopTitle" :maxlength="100"
                            showWordLimit />
                    </el-form-item>

                    <el-form-item label="门店公告" prop="description">
                        <TigInput width="90%" placeholder="请填写门店公告" v-model="formState.description" type="textarea"
                            :maxlength="200" showWordLimit :rows="4" />
                        <div class="extra">请填写门店简介，该简介会显示在用户门店详细中</div>
                    </el-form-item>

                    <el-form-item label="门店地址" prop="shopRegionIds" :rules="[{ required: true, message: '请选择门店地址!' }]">
                        <div>
                            <SelectRegion v-if="action == 'add' || !loading"
                                v-model:selectedIds="formState.shopRegionIds" @change="onRegionChange" @clear="onClear"
                                style="width: 260px; margin-bottom: 10px"></SelectRegion>
                            <TigInput width="500px" placeholder="详细地址" v-model="formState.shopDetailedAddress"
                                @Input="updateMapInfo" />
                            <div class="map-container">
                                <MapContainer ref="MapRef" :defaultPosition="defaultPosition"
                    ...
                    <el-form-item label="门店标签" prop="shopTips">
                        <div>
                            <div v-if="selectedshopTips">
                                <el-tag v-for="(item, index) in selectedshopTips" :key="index" class="mr10">{{
                                    item.tipName }}</el-tag>
                            </div>
                            <div>
                                <DialogForm :params="{ selectedIds: selectedshopTips }" isDrawer
                                    path="store/adminPages/storeTipManage/src/SelectTips" title="选择标签" width="600px"

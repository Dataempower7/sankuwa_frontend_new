    ...
    <el-form ref="formRef" :model="formState" label-width="auto"
        ...
        <div class="content_wrapper">
            ...
            <el-form-item label="门店独立商品" prop="storeIndependentGoods">
                <div>
                    <el-checkbox v-model="formState.storeIndependentGoods" label="允许门店自建商品" :true-label="1"
                        :false-label="0" />
            ...
            <el-form-item label="门店商品审核" prop="shopProductNeedCheck" v-if="formState.storeIndependentGoods == 1">
                <div>
                    <el-checkbox v-model="formState.shopProductNeedCheck" label="商品默认审核通过" :true-label="0"
                        :false-label="1" />
            ...
            <el-form-item label="分配商品名称" prop="storeAssignProductName">
                <div>
                    <el-checkbox v-model="formState.storeAssignProductName" label="门店可修改商品名称" :true-label="1"
                        :false-label="0" />
                    <div class="extra">
            ...
            <el-form-item label="库存模式" prop="storeUseTotalProductStock">
                <el-checkbox v-model="formState.storeUseSoloProductStock"
                    :disabled="formState.storeUseSoloProductStock === 1 && formState.storeUseTotalProductStock === 0 ? true : false"
                    label="使用门店独立库存" :true-label="1" :false-label="0" />
                <el-checkbox v-model="formState.storeUseTotalProductStock"
                    :disabled="formState.storeUseTotalProductStock === 1 && formState.storeUseSoloProductStock === 0 ? true : false"
                    label="使用平台库存" :true-label="1" :false-label="0" />
            </el-form-item>
            <el-form-item label="客户下单配送方式" prop="storeCustomSubmitShippingType">
                <div>
                    <el-radio-group v-model="formState.storeCustomSubmitShippingType">
                        ...
                        <el-radio :value="2">同城优先</el-radio>
                    </el-radio-group>
                    <div class="extra">
                        商品存在多种配送方式时，客户下单，默认配送方式为您勾选的方式，客户可在订单提交页面手动修改配送方式。<el-popover :width="400" placement="right-end"
                            trigger="click">
                            <template #reference>
                                <a>查看示例</a>
                            </template>
                            <template #default>
                                <img src="@/style/images/store/storeCustomSubmitShippingType.png"
                                    style="width: 380px" />
                            </template>
                        </el-popover>
                    </div>

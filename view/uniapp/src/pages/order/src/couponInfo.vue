<template>
    <view class="coupon-info">
        <template v-if="flowType != 3">
            <template v-if="configStore.useCoupon == 1">
                <view class="coupon">
                    <view class="coupon-title" style="color: #666;">{{ $t("优惠券") }}</view>
                    <view class="coupon-content" @click="handleCoupon">
                        <template v-if="couponAmount > 0">
                            <view class="coupon-text" style="color: #ff4757; font-weight: bold;">{{ $t("优惠：") + currency + couponAmount }}</view>
                        </template>
                        <template v-else>
                            <view class="coupon-text" style="color: #999;">{{ $t("暂无优惠") }}</view>
                        </template>
                        <image src="/static/images/common/right.png" style="width: 15px; height: 15px;margin-left: 10rpx;  opacity: 0.6;"/>
                    </view>
                </view>
            </template>
            <template v-if="configStore.usePoints == 1">
                <view class="points">
                    <view class="points-title" style="color: #666;"
                        >{{ $t("积分") }}
                        <view class="points-text" style="color: #3B49A9;"
                            >{{ $t("当前账户积分") }}：<text class="special-text" style="color: #3B49A9;">{{ points }}</text>
                        </view>
                    </view>
                    <view class="points-content" @click="handlePoints">
                        <template v-if="pointsAmount > 0">
                            <view class="points-text" style="color: #ff4757; font-weight: bold; font-size: 28rpx;">{{ $t("抵扣：") }}{{ currency + pointsAmount }}</view>
                        </template>
                        <template v-else>
                            <view class="points-text" style="color: #999;">{{ $t("暂无积分优惠") }}</view>
                        </template>
                        <image src="/static/images/common/right.png" style="width: 15px; height: 15px;margin-left: 10rpx;  opacity: 0.6;"/>
                    </view>
                </view>
    ...
    <tig-popup v-model:show="showPoints" background-color="#f5f5f5">
        <view class="points-popup">
            <view class="popup-title">{{ $t("积分") }}</view>
            <view class="form-usePoints">
                <uni-forms ref="pointsForm" :model-value="formData">
                    <uni-forms-item label-width="80" :label="$t('使用积分')" name="usePoints">
                        <uni-easyinput
...
const handleCheck = (item: any) => {
    ...
    // console.log("selectedDatas", selectedDatas.value);
};
...
const rules = {
    usePoints: {
        rules: [
            { required: true, errorMessage: t("请输入积分") },
            {
                validateFunction: function (rule: any, value: number, data: any, callback: any) {
                    if (value > props.availablePoints) callback(t("积分不能大于可用积分"));
                    if (value < 0) callback(t("积分不能小于0"));
                    return true;
                }
            }
        ]

<template>
    <view v-for="(item, index) in cartList" :key="index" class="store-card">
        <view class="store-card-item">
            ...
            <template v-for="(subItem, subIndex) in getDisplayCarts(item.carts, item.shopId)" :key="subIndex">
            ...
            <!-- 展开/收起按钮 -->
            <view v-if="item.carts.length > 3" class="expand-toggle" style="display: flex;
            justify-content: center;
            align-items: center;
            padding: 20rpx 0;
            margin-top: 20rpx;
            border-top: 1rpx solid #f5f5f5;
            cursor: pointer;" @click="toggleExpand(item.shopId)">
                <template v-if="isExpanded(item.shopId)">
                    <text class="expand-text" style=" font-size: 26rpx;
                color: #8F8F8F;
                text-align: center;">收起 (共{{ item.carts.length }}件)</text>
                    <image src="/static/images/common/up.png" style=" width: 30rpx;height: 30rpx;opacity: 0.6;     margin-left: 10rpx;" />
                </template>
                <template v-else>
                    <text class="expand-text" style=" font-size: 26rpx;
                color: #8F8F8F;
                text-align: center;">展开 (共{{ item.carts.length }}件)</text>
                    <image src="/static/images/common/down.png" style=" width: 30rpx; height: 30rpx;opacity: 0.6;    margin-left: 10rpx;" />
                </template>
            </view>
            ...
            <view class="shipping-fee">
                <view class="shipping-fee-title">快递运费</view>
                <view class="shipping-fee-content">
                    <template v-if="(props.shippingFee || 0) > 0">
                        <format-price :price-data="props.shippingFee || 0" />
                    </template>
                    <template v-else>
...
// 展开/收起相关方法
const isExpanded = (shopId: number) => {
...
const getDisplayCarts = (carts: any[], shopId: number) => {
    if (carts.length <= 3) {
        return carts;
    }
    return isExpanded(shopId) ? carts : carts.slice(0, 3);
};
